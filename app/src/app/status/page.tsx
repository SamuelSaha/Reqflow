    ...
    <PageShell>
      <section className="pt-20 pb-20 px-20">
        <div className="max-w-[900px] mx-auto">
          <h1 className="text-[48px] font-extrabold tracking-tight text-slate-900 mb-4">System Status</h1>
          <p className="text-[15px] text-slate-500 mb-12">Real-time status for Reqflow services · Last updated: {lastUpdate}</p>
          ...
          <div className={`rounded-2xl p-8 mb-12 text-center border ${allOperational ? "bg-emerald-50 border-emerald-200" : "bg-amber-50 border-amber-200"}`}>
            <div className="text-[48px] mb-4">{allOperational ? "\u2713" : "\u26A0"}</div>
          ...
          <div className="bg-amber-50 border border-amber-200 rounded-xl p-6 mb-12">
            <h3 className="text-[18px] font-bold text-amber-600 mb-3">Beta Status Monitoring</h3>
            <p className="text-[15px] text-slate-600 leading-[1.7]">
          ...
          <div className="mb-12">
            <h2 className="text-[24px] font-bold text-slate-900 mb-6">Service Status</h2>
              ...
              {services.map((service) => (
                <div key={service.name} className="bg-slate-50 border border-slate-200 rounded-xl p-6 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                    <div className={`w-10 h-10 rounded-[10px] border flex items-center justify-center text-[18px] ${statusBg(service.status)} ${statusColor(service.status)}`}>
                  ...
                  <div className="text-right">
                    <div className="text-[13px] text-slate-500 mb-0.5">30-day uptime</div>
                    <div className="text-[18px] font-bold text-slate-900">{service.uptime}</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="mb-12">
            <h2 className="text-[24px] font-bold text-slate-900 mb-6">Infrastructure</h2>
          ...
          <div className="bg-slate-50 border border-slate-200 rounded-2xl p-10 text-center">
            <h3 className="text-[22px] font-bold text-slate-900 mb-3">Get Status Updates</h3>
            <p className="text-[15px] text-slate-600 mb-6">Beta customers are automatically added to our shared Slack channel for real-time incident updates. Email notifications for major outages coming soon.</p>
            <a href="/beta" className="inline-block bg-blue-600 text-white py-3 px-7 rounded-lg text-[15px] font-bold no-underline hover:bg-blue-700 transition-colors">
              Join Beta Program
            </a>
          </div>
        </div>
